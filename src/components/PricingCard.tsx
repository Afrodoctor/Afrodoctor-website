...
const PricingCard: React.FC<PlanProps> = ({ name, price, features, isPrimary }) => {
  const [buttonText, setButtonText] = useState("Get Started");
  const [isHovered, setIsHovered] = useState(false);

  const handleCtaClick = () => {
    setButtonText("Request Sent! ✓");
    console.log(`Plan interest submitted: ${name}`);
    setTimeout(() => setButtonText("Get Started"), 3000);
  };

  const showPerMonth = price && !/contact/i.test(price);
  ...
  return (
    <div
      className={`
        ...
        ${isPrimary
          ? 'bg-blue-900 dark:bg-blue-800 text-white scale-105 border-2 border-blue-400 dark:border-blue-500'
    ...
    >
      ...
      <div className="relative z-10 flex-1">
        {/* Plan name */}
        <h3 className={`text-3xl font-extrabold mb-2 ${isPrimary ? 'text-white' : 'text-gray-900 dark:text-white'}`}>
          {name}
        </h3>
       
        {/* Subtitle */}
        <p className={`mb-6 text-sm ${isPrimary ? 'text-blue-200' : 'text-gray-500 dark:text-gray-400'}`}>
          {isPrimary ? 'Ideal for scaling health systems' : name.includes('Enterprise') ? 'For large organizations' : 'Perfect for small clinics'}
        </p>

        {/* Price */}
        <div className="mb-8">
          <p className="text-5xl font-extrabold" aria-hidden={false}>
            {price}
            {showPerMonth && (
              <span className={`text-xl font-medium ${isPrimary ? 'text-blue-200' : 'text-gray-500 dark:text-gray-400'}`}>/month</span>
            )}
          </p>
        </div>
        ...
        <ul className="space-y-4 mb-10">
          {features.map((feature, index) => (
            <li key={index} className="flex items-start group/feature">
              <div className={`
                flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center mr-3 mt-0.5 transition-all duration-300
                ${isPrimary
              ...
              `}>
                <svg
                  className={`w-4 h-4 ${isPrimary ? 'text-white' : 'text-blue-800 dark:text-blue-400'}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
              ...
              <span className={`leading-relaxed transition-all duration-300 ${isPrimary ? 'text-blue-50 group-hover/feature:text-white' : 'text-gray-700 dark:text-gray-300 group-hover/feature:text-gray-900 dark:group-hover/feature:text-white'}`}>
                {feature}
              </span>
            </li>
          ))}
